IDE_Morph.prototype.createSpriteBar = function () {
    ...
    if (cnf.dynaverseMode) {
        rotationStyleButtons.forEach(each => each.hide());
        thumbnail.hide();
        nameField.hide();
        padlock.hide();
    }
...
IDE_Morph.prototype.fixLayout = function (situation) {
    ...
    if (situation !== 'refreshPalette') {
        ...
        if (this.isEmbedMode) {
            ...
            this.stage.setCenter(new Point(this.center().x, this.center().y + this.controlBar.height() / 2));
        } else {
        ...
        this.spriteBar.setHeight(
            cnf.dynaverseMode ?
                Math.round(this.logo.height() * 0.7)
...
IDE_Morph.prototype._toggleAppMode = IDE_Morph.prototype.toggleAppMode;
IDE_Morph.prototype.toggleAppMode = function (appMode) {
    this._toggleAppMode(appMode);
    if (this.isAppMode && this.stage.inputManager) {
        const ext = this.world().extent();
        this.stage.setExtent(new Point(ext.x, ext.y - this.controlBar.height()));
        this.fixLayout();
        this.stage.inputManager.publish("input", "resize");
    }
};
