...
SpriteMorph.prototype.createCardClone = function () {
    const cardId = this.getCardProperty("id");
    const args = [cardId, this.name];
...
SpriteMorph.prototype.removeCardClone = function () {
    const cardId = this.getCardProperty("id");
    this.removeClone();
    this.publish('spriteManager', 'removeCard', cardId);
};

SpriteMorph.prototype.doPublish = function (event, options) {
    const scope = Process.prototype.inputOption(options.at(1)) || 'id';
    let data = options.at(2);
    if (event) {
        if (data) {
            if (data instanceof List) {
                // 1 dimensional list (from Snap!)
                data = data.asArray();
            }
...
SnapExtensions.primitives.set(
    // publishAndWait
    'publishAndWait(scope, event, data)',
    function (scope, event, data, proc) {
        if (!proc.context.accumulator) {
                ...
                if (!this._accumulatorWait){
                    // SnapExtensions.primitives(class) scope
                    this._accumulatorWait = {};
                }
                // card scope, not SnapExtensions.primitives(class) scope
                ...
                if (card) {
                    ...
                    if (scope === 'id') {
                        event = `blocks:${event}`;
                        this.cardPawn.say(event, data);
                        // console.log('block:doneMessage to microverse', this.cardPawn.actor.id);
                    } else {
                        this.cardPawn.publish(scope, event, data);
                    }
                    console.log("cardPawn", this.cardPawn, "publish", scope, event, data);
                    this._accumulatorWait[messageId] = proc;
                    /*
                    const timeout = 2000;
                    setTimeout(() => {
                        if (this._accumulatorWait[messageId]){
...
SnapExtensions.primitives.set(
  ...
  function (scope, event, data, proc) {
    const messageId = Math.floor(Math.random() * 1000000000); // unused in the subscribe
    const card = this.cardPawn?.actor;
    if (card) {
      data = data.asArray();
      data = [messageId, data];
...
SpriteMorph.prototype._handleDoneMessage = function (messageId) {
    console.log("_handleDoneMessage", Object.keys(this._accumulatorWait).length, this._accumulatorWait, messageId);
    if (this._accumulatorWait[messageId]){
        this._accumulatorWait[messageId].context.accumulator.done = true;
        delete this._accumulatorWait[messageId];
    }
}
